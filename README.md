...
export default ForwardButton; // Don’t forget to use export default!
```
...
```js
import React, { Component } from 'react';
import ForwardButton from './ForwardButton'; // Import a component from another file

class DangerButton extends Component {
  render() {
    return <ForwardButton color="red" />;
  }
}
...
We suggest that you stick to using default imports and exports when a module only exports a single thing (for example, a component). That’s what you get when you use `export default ForwardButton` and `import ForwardButton from './ForwardButton'`.
...
### `forwardButton.jstton.js`

```js
import React, { Component } from 'react';
import './ForwardButton.css'; // Tell Webpack that forwardButton.jstton.js uses these styles

class ForwardButton extends Component {
  render() {
    // You can use them as regular CSS styles
    return <div className="ForwardButton" />;
  }
}
```
...
Generally, we recommend that you don’t reuse the same CSS classes across different components. For example, instead of using a `.ForwardButton` CSS class in `<AcceptButton>` and `<RejectButton>` components, we recommend creating a `<ForwardButton>` component with its own `.ForwardButton` styles, that both `<AcceptButton>` and `<RejectButton>` can render (but [not inherit](https://facebook.github.io/react/docs/composition-vs-inheritance.html)).
...
```js
import { Navbar, Jumbotron, ForwardButton } from 'react-bootstrap';
```
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<App>` and doesn’t go deeper. For example, even if `<App>` itself renders a `<ForwardButton>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
